"""
Meallion Voice AI - Elena Greek Agent (Patch 5 el)
Full version with fixes for double responses, dead states, and transcript formatting.
Adapted for Greek language based on patch5.py and elena_el.py.
"""
...
def _clean_transcript_phone(text: str) -> str:
    """Clean transcript text by replacing formatted phone numbers with pure digits."""
    cleaned = text
...
def snooze_silence(seconds: float):
    """Globally accessible snooze function."""
...
def _mentions_order_lookup_intent(text: str) -> bool:
    ...
    t = (text or "").lower()
    return bool(re.search(r"(αριθμός παραγγελίας|κωδικός παραγγελίας|έλεγχος παραγγελίας|άλλη παραγγελία)", t))
...
class ElenaFunctionContext(llm.FunctionContext):
    ...
    async def lookup_order(self, order_number: Annotated[str, llm.TypeInfo(description="Order number")]) -> str:
        ...
        if len(clean_num) >= 7:
            return "ΣΦΑΛΜΑ: Αυτό το εργαλείο είναι μόνο για 3-6 ψήφιους αριθμούς παραγγελίας. Για αριθμούς τηλεφώνου, παρακαλώ περιμένετε τον αυτόματο έλεγχο."
    ...
    async def lookup_order_by_phone(self, phone: Annotated[str, llm.TypeInfo(description="10-digit phone number")]) -> str:
        ...
        if len(clean_phone) < 10:
            return "ΣΦΑΛΜΑ: Αυτό το εργαλείο απαιτεί πλήρη 10ψήφιο αριθμό τηλεφώνου."
...
async def _run_order_lookup(agent: VoicePipelineAgent, order_number: str):
    ...
    try:
        ...
        await agent.say("Λυπάμαι, μου παίρνει λίγο περισσότερο χρόνο από το συνηθισμένο για να βρω την παραγγελία σας. Μπορείτε να μου επαναλάβετε τον αριθμό παραγγελίας;", allow_interruptions=True)
        state.support_state = "awaiting_order"
    except Exception as e:
        logger.error("Order lookup error: %s", e)
        await agent.say("Λυπάμαι, δυσκολεύομαι να ελέγξω αυτόν τον αριθμό παραγγελίας αυτή τη στιγμή. Παρακαλώ δοκιμάστε ξανά σε λίγο.", allow_interruptions=True)
        state.support_state = "awaiting_order"
    finally:
...
async def _run_phone_lookup(agent: VoicePipelineAgent, phone_number: str):
    ...
    try:
        ...
        await agent.say("Λυπάμαι, δυσκολεύομαι λίγο να το βρω. Μπορείτε να μου πείτε τον αριθμό τηλεφώνου άλλη μια φορά;", allow_interruptions=True)
        state.support_state = "awaiting_phone"
    except Exception as e:
        logger.error("Phone lookup error: %s", e)
        await agent.say("Λυπάμαι, δυσκολεύομαι να ελέγξω αυτόν τον αριθμό τηλεφώνου αυτή τη στιγμή. Παρακαλώ δοκιμάστε ξανά σε λίγο.", allow_interruptions=True)
        state.support_state = "awaiting_phone"
    finally:
        state.lookup_inflight = False
    ...
    def _on_user_speech_committed(msg):
        ...
        if 7 <= len(all_digits) <= 9 and state.support_state == "awaiting_phone":
            ...
            asyncio.create_task(agent.say(
                f"Άκουσα έναν αριθμό με {len(all_digits)} ψηφία. Θα μπορούσατε παρακαλώ να μου δώσετε τον πλήρη 10ψήφιο αριθμό τηλεφώνου;",
                allow_interruptions=True
            ))
            return
        ...
        support_intent = bool(re.search(r"(problem|issue|complaint|order problem|wrong order|late order|my order|πρόβλημα|θέμα|παράπονο|καθυστέρηση)", user_text.lower()))
        if support_intent:
            state.support_state = "awaiting_order"
        ...
        ticket_intent = bool(re.search(r"(human|representative|call me|callback|support ticket|open ticket|create ticket|εκπρόσωπο|άνθρωπο|επικοινωνία)", user_text.lower()))
        if ticket_intent:
            state.support_state = "ticket_name"
            suppress_llm(15.0)
    ...
    async def _silence_monitor():
        ...
        while not state.should_end:
            ...
            if state.support_state == "awaiting_phone":
                text = "Είμαι ακόμα έτοιμη να βοηθήσω. Παρακαλώ πείτε μου το τηλέφωνο για την παραγγελία όποτε μπορείτε."
