"""
Meallion Voice AI - Elena English Agent (Patch 2)
Fixed version with refined intent detection and prioritized data extraction.
"""
...
def _mentions_phone_lookup_intent(text: str) -> bool:
    t = (text or "").lower()
    # PATCH 2: Be much more specific. Only trigger if "phone" or "mobile" is explicitly mentioned
    # or if they ask to check "by phone number". Generic "number" is too ambiguous.
...
async def entrypoint(ctx: JobContext):
    ...
    def _on_user_started_speaking():
        cancel_thinking_task()
        state.waiting_for_user = False
        # PATCH 2: Immediately snooze silence monitor when user starts speaking
        # to prevent "stale" silence prompts from overlapping with user turn.
        snooze_silence(5.0)
    ...
    def _on_user_speech_committed(msg):
        ...
        if state.support_state in {"awaiting_order", "checking_order"}:
            # PATCH 2: ALWAYS prioritize valid digits.
        ...
        if state.support_state in {"awaiting_phone", "checking_phone"}:
            # PATCH 2: Prioritize phone number digits over generic intent.
    ...
    async def _silence_monitor():
        while not state.should_end:
            ...
            # PATCH 2: Double check that we are still in a state that warrants a prompt
            # and that we haven't just started speaking or listening.
            ...
            # Suppress LLM while the deterministic silence prompt is playing.
            suppress_llm(15.0)
...
def prewarm(proc: JobProcess):
    logger.info("Elena EN Patch 2 prewarm complete")
