...
def _speak_digits(raw: str) -> str:
    """Convert digits into Greek spoken words digit-by-digit for reliable confirmations."""
...
def _is_affirmative_utterance(text: str) -> bool:
    ...
    yes_tokens = {
        ...
        "ναι ευχαριστω", "ναι παρακαλώ", "βεβαιως", "βεβαίως",
        "ακριβως", "ακριβώς", "εντάξει", "ε εντάξει",
    }
    # Exact match first
    if normalized in yes_tokens:
        return True
    # Starts with any yes token (handles "ναι είναι σωστός" etc.)
...
def _is_negative_utterance(text: str) -> bool:
    """Return True for short negative confirmations like 'όχι'."""
    normalized = _normalize_intent_text(text)
    if not normalized:
        return False
    no_tokens = {"οχι", "όχι", "οχι ευχαριστω", "όχι ευχαριστώ", "οχι παρακαλω"}
...
def create_tts():
    ...
    # Always use multilingual model for Greek support
    tts_model = "eleven_multilingual_v2"
    logger.info("TTS model selected: %s (language: el)", tts_model)
...
def create_stt(*, is_sip_call: bool = False):
    ...
    stt_lang = get_stt_language('el')  # Greek-only: always use Greek STT
    stt_auto_detect = False  # Greek-only: no auto language detection
    sip_stt_auto_detect = _as_bool(
        get_agent_setting("sip_stt_auto_detect", False),
        default=False,
    )
    effective_stt_auto_detect = False  # Greek-only: never auto-detect language
    ...
    # Force Greek transcription bias
    openai_stt_prompt = "Transcribe exactly what is spoken. Keep the original spoken language. Likely language is Greek."
    ...
    if provider == "deepgram" and USE_DEEPGRAM:
        use_auto_detect = False  # Greek-only: no auto language detection
    ...
    # Fallback to OpenAI Whisper (Greek language)
...
class ElenaFunctionContext(llm.FunctionContext):
    ...
    async def lookup_order(
        ...
        try:
            ...
            summary = _build_order_voice_summary(result, 'el') or result
            return summary
        finally:
    ...
    async def get_order_details(
        ...
        try:
            ...
            spoken_summary = _build_order_details_voice_summary(result, 'el')
            if not spoken_summary:
                spoken_summary = _build_order_voice_summary(result, 'el') or (
    ...
    async def lookup_order_by_phone(
        ...
        try:
            ...
            summary = _build_phone_lookup_voice_summary(result, 'el') or result
            return summary
        finally:
...
async def entrypoint(ctx: JobContext):
    ...
    logger.info(f"Elena EL agent starting for room: {ctx.room.name} (job: {ctx.job.id})")
    ...
    def _enforce_order_status(text: str) -> str:
        ...
        language = last_lookup_status["language"] or 'el'
            ...
            if isinstance(text, str):
                ...
                if _is_lookup_wait_ack_only_text(text):
                    ...
                    if not lookup_started:
                        ...
                        replacement = _repeat_number_prompt_for_mode(mode, 'el')
                        _current_session["pending_lookup_wait_phrase"] = None
                        _current_session["pending_lookup_wait_phrase_set_at"] = 0.0
                        invalid_recovery_grace = _as_float(
                            get_agent_setting("invalid_number_recovery_silence_grace_seconds", 12.0),
                ...
                # Language lock removed: Greek-only module
    ...
    logger.info("Language mode: Greek-only (no auto switch), call_type=%s", call_type)
    ...
    async def _force_lookup_order_by_number(turn_id: int, order_number: str, scheduled_at: float) -> None:
        ...
        try:
            ...
            spoken_summary = _build_order_voice_summary(result, 'el') or result
    ...
    async def _force_lookup_by_phone(turn_id: int, phone: str, trigger_reason: str, scheduled_at: float = 0.0) -> None:
        ...
        try:
            ...
            wait_msg = "Μισό λεπτό, ψάχνω την παραγγελία σας."
            live_agent = _current_session.get("agent")
            ...
            spoken_summary = _build_phone_lookup_voice_summary(result, 'el') or result
            await send_agent_transcript(spoken_summary)
            agent.chat_ctx.append(role="assistant", text=spoken_summary)
            if live_agent:
                await live_agent.say(spoken_summary, allow_interruptions=True)
    ...
    def on_user_speech_committed(message):
        ...
        set_runtime_language("el")  # Greek-only module: always enforce Greek
            ...
            if (
                ...
                if (
                    ...
                    agent.chat_ctx.append(
                        role="system",
                        text=(
                            "ΠΡΟΤΕΡΑΙΟΤΗΤΑ ΑΝΑΖΗΤΗΣΗΣ ΠΑΡΑΓΓΕΛΙΑΣ:\n"
                            f"- Ο καλέσας παρείχε ήδη τον αριθμό παραγγελίας {detected_order_number}.\n"
                            "- Στην επόμενη απάντησή σας, καλέστε αμέσως το lookup_order με αυτόν τον ακριβή αριθμό.\n"
                            "- Μην ζητήσετε ξανά τον αριθμό παραγγελίας εκτός αν το lookup_order επιστρέψει άκυρο ή μη εύρεση.\n"
                            "- Μόλις το εργαλείο επιστρέψει, δώστε την κατάσταση χωρίς επιπλέον καθυστέρηση."
                        ),
                    )
                    room_log("ORDER_LOOKUP_HINT_INJECTED", order_number=detected_order_number)
            ...
            # --- 2. Phone Number Watchdog ---
                ...
                if (
                    ...
                    agent.chat_ctx.append(
                        role="system",
                        text=(
                            "ΠΡΟΤΕΡΑΙΟΤΗΤΑ ΑΝΑΖΗΤΗΣΗΣ ΤΗΛΕΦΩΝΟΥ:\n"
                            f"- Ο καλέσας παρείχε τον αριθμό τηλεφώνου {detected_phone}.\n"
                            "- Στην επόμενη απάντησή σας, καλέστε αμέσως το lookup_order_by_phone με αυτόν τον αριθμό τηλεφώνου.\n"
                            "- Μην ζητήσετε επιβεβαίωση ή επανάληψη του αριθμού εκτός αν το εργαλείο αποτύχει.\n"
                            "- Μόλις το εργαλείο επιστρέψει, δώστε την κατάσταση χωρίς επιπλέον καθυστέρηση."
                        ),
                    )
                    room_log("PHONE_LOOKUP_HINT_INJECTED", phone=detected_phone)
    ...
    def on_agent_speech_committed(message):
        ...
        try:
            ...
            if text:
                normalized_text = text  # Greek-only: no language enforcement needed
        ...
        for fn in called_functions:
            ...
            if name == "lookup_order":
                ...
                if snapshot:
                    ...
                    last_lookup_status["language"] = 'el'
                ...
                if lookup_active:
                    ...
                    if last_progress and (now - last_progress) >= progress_interval:
                        ...
                        if not silence_tracker.get("user_is_speaking") and not _current_session.get("forced_response_manual_say_active"):
                            ...
                            progress_msg = "Ακόμη ψάχνω για την παραγγελία σας, ευχαριστώ για την υπομονή σας."
