...
async def entrypoint(ctx: JobContext):
    ...
    def _mentions_no_order_number(text: str) -> bool:
        ...
        # Removed standalone "no" check as it's too aggressive (matches "no more details" etc)
        return (has_negative and (has_order_keyword or has_have_keyword))
    ...
    async def send_agent_transcript(text: str, *, source: str = "manual"):
        ...
        try:
            ...
            if call_ended["value"]:
                return
            # Allow both manual (immediate) and commit (final) transcripts.
            # We de-duplicate them below using a window.
            ...
            # Use a longer window for committed transcripts to avoid duplicates from manual paths
            # (which were sent at the start of speech, while commit happens at the end).
            dedup_window = 25.0 if source == "commit" else 3.0
            if cleaned == last_sent_text and (time.time() - last_sent_at) < dedup_window:
                return
    ...
    def _is_details_retry_fallback_text(text: str) -> bool:
        ...
        return bool(
            re.search(
                r"(issue retrieving the details|trouble getting the detailed information|let me try again|let me try that again)",
                normalized,
                flags=re.IGNORECASE,
            )
        )
    ...
    # Preemptive synthesis disabled to ensure deterministic memory-first priority.
    # This prevents the LLM from speaking before on_user_speech_committed logic settles.
    preemptive_synthesis = False
    logger.info("TTS preemptive_synthesis=False (Enforced)")
    ...
    async def _force_get_order_details(turn_id: int, trigger_reason: str) -> None:
        ...
        try:
            ...
            # Shrink suppression window now that we've started speaking the result.
            _current_session["forced_response_suppress_llm_until"] = time.time() + 10.0
    ...
    def on_user_speech_committed(message):
        ...
        # Publish user transcript immediately to the room for UI responsiveness
        ...
        # Only allow "no order number" recovery if we are in a state that expects one.
        # This prevents a "No" response to order-details prompts from triggering phone flow.
        no_order_recovery = False
        if flow_state in {FLOW_AWAITING_ORDER_NUMBER, FLOW_CHECKING_ORDER_NUMBER, FLOW_IDLE}:
        ...
        user_text_for_transcript = _format_user_text_for_transcript(user_text)
        # Already sent at the beginning of on_user_speech_committed
                ...
                if suppress_commit:
                    ...
                    keep_core_message = (
                        ...
                        or bool(
                            ...
                            re.search(
                                r"(checking|searching|looking up|one moment|moment please|wait a second)",
                                _normalize_switch_text(display_text or text),
                                flags=re.IGNORECASE,
                            )
                        )
