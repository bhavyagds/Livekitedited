"""
Meallion Voice AI - Elena English Agent (Patch 1)
Fixed version of Elena English agent with prioritized order recognition.
"""
...
def _mentions_phone_lookup_intent(text: str) -> bool:
    t = (text or "").lower()
    # PATCH 1: If "number" is mentioned, ensure it's not "order number"
    has_generic_number = bool(re.search(r"\bnumber\b", t) and not re.search(r"order\s+number", t))
    return bool(
        re.search(r"\b(phone|mobile|call me on)\b", t)
        or has_generic_number
...
async def entrypoint(ctx: JobContext):
    ...
    def _on_user_speech_committed(msg):
        ...
        if state.support_state in {"awaiting_order", "checking_order"}:
            # PATCH 1: High priority: Check for a valid order ID first.
            # This ensures "My order number is 12345" is caught here even if it contains "number".
...
def prewarm(proc: JobProcess):
    logger.info("Elena EN Patch 1 prewarm complete")
