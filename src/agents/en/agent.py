"""
Meallion Voice AI - Elena (Local Standalone Agent)
...
logger = logging.getLogger("agent-elena")
...
def create_tts():
    ...
    voice_id = str(
        get_agent_setting("agent_voice_id", getattr(settings, "elevenlabs_voice_id", ""))
...
FALLBACK_INSTRUCTIONS = (
    ...
    "Reply in plain English. Be warm, direct, and empathetic. "
...
@server.rtc_session(agent_name="meallion-agent-en", on_request=request_fnc)
async def entrypoint(ctx: JobContext):
    ...
    try:
        ...
        logger.warning("English agent: language guard DB check failed (%s) — proceeding anyway", _e)
        _lang = "en"  # default to English on error

    if _lang != "en":
        logger.info(
            "English agent: active language is '%s', not 'en' — calling shutdown to release job",
            _lang,
        )
        await ctx.shutdown()  # Properly release the job back to LiveKit
        return

    logger.info("English agent: language='en' confirmed — starting session")
    ...
    # ------------------------------------------------------------------
    # 3. Detect call type: SIP rooms are named "sip-call-+XXXXXXXXXXX"
    # ------------------------------------------------------------------
    ...
    def _on_user_input_transcribed(ev):
        if ev.is_final and ev.transcript:
            ...
            end_phrases = [
                ...
                "thank you", "thanks", "no thanks", "no thank you",
                "that's all", "that is all", "that's it", "thats all", "thats it"
            ]
            words = cleaned.split()
