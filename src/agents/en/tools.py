...
async def create_ticket_without_phone(
    ...
    """
    Create a support ticket without a phone number (phone will be collected and added separately).
...
async def update_ticket_with_phone(task_id: str, phone: str) -> dict:
    """
    Update an existing support ticket with the customer's phone number.
    ...
    if result.get("success"):
        return {"success": True, "message": f"Phone number {cleaned} added to ticket."}
    return {"success": False, "message": "Could not update ticket with phone number."}
