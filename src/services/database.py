...
class DatabaseService:
    ...
    async def record_call_start(self, room_name: str, call_type: str = "web", caller_number: str = None, caller_identity: str = None) -> Optional[str]:
    ...
    async def record_call_end(self, call_id: str = None, room_name: str = None, status: str = "completed", duration_seconds: int = None, disconnect_reason: str = None, transcript: str = None) -> bool:
        """Record the end of a call."""
        try:
            async with get_db() as session:
                if call_id:
                    stmt = update(Call).where(Call.id == uuid.UUID(call_id))
                elif room_name:
                    stmt = update(Call).where(Call.room_name == room_name, Call.status == "active")
                else:
                    return False
    ...
    async def update_call_transcript(self, call_id: str, transcript: str, append: bool = True) -> bool:
        """Update or append to a call's transcript."""
        try:
            async with get_db() as session:
                if append:
                    # Append logic: first get existing then update
                    result = await session.execute(select(Call).where(Call.id == uuid.UUID(call_id)))
                    call = result.scalar_one_or_none()
                    if call:
                        existing = call.transcript or ""
                        new_transcript = f"{existing}\n{transcript}".strip() if existing else transcript
    ...
    async def get_memory_item(self, item_id: str) -> Optional[Dict]:
        """Get a single long-term memory entry by ID."""
        try:
            async with get_db() as session:
                result = await session.execute(
                    select(AgentMemory).where(AgentMemory.id == uuid.UUID(item_id))
                )
    ...
    async def update_memory_item(
        ...
        try:
            ...
            logger.error(f"Error updating memory item {item_id}: {e}")
            return False
